...
{
public:	
	MinSpanTree(int sz=DefaultSize-1):maxSize(sz),n(0)		//构造函数
	{
		edgevalue=new MSTEdgeNode<T,E>[sz];
	}
	void Insert(MSTEdgeNode<T,E> &item);					//插入边
	bool isEmpty(){return (n==0)?true:false;}				//是否是空
	void show()												//输出边
	{
		for(int i=0;i<n;i++)
		{
	...
	int getTail(int i) { return edgevalue[i].tail; }	//得到第i条边的起点
	int getKey(int i) { return edgevalue[i].key; }		//得到第i条边的值
	int getHead(int i) { return edgevalue[i].head; }	//得到第i条边的终点
private:
	MSTEdgeNode<T,E> *edgevalue;	//边数组
	int maxSize,n;					//最大大小和当前大小
};
